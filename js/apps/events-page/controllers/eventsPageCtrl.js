        ...
        vm.filterEvents = (eventRequestModel, isInit, callback) => {
            ...
            startDatePicker &&
                startDatePicker.setDate(
                    $window.moment(eventRequestModel.StartDate).toDate()
                ); // eslint-disable-line no-unused-expressions
            endDatePicker &&
                endDatePicker.setDate(
                    $window.moment(eventRequestModel.EndDate).toDate()
                ); // eslint-disable-line no-unused-expressions
            ...
            eventsService
                ...
                .then(events => {
                    ...
                    const filterPanelStatuses = getFilterPanelStatus(
                        eventRequestModel
                    );

                    if (isInit) {
                        bootstrapCollapseHelper.toggleCollapseByIds(
                            filterPanelStatuses
                        );
                    } else {
        ...
        const combineEventGroups = (oldEventGroups, newEventGroups) => {
            ...
            angular.forEach(newEventGroups, eventGroup => {
                if (isSameDay(lastEventGroup.date, eventGroup.date)) {
                    lastEventGroup.events = lastEventGroup.events.concat(
                        eventGroup.events
                    );
                } else {
        ...
        const getRequestModelBasedOnQueryParams = () => {
            ...
            queryParamKeys.forEach(filterType => {
                ...
                if (!isFilterTypeDate) {
                    ...
                    const filterValues = filterHelperService.getFiltersFromString(
                        filterValStr
                    );
                    ...
                    filterValues.forEach(filterVal => {
                        ...
                        if (filterId && filterId > -1) {
                            let requestModelKey = upperCaseFirstLetter(
                                filterType
                            );
