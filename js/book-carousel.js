...
bcpl.bookCarousel = (($, constants) => {
  ...
  const slickSettings = {
    ...
    prevArrow:
      '<a href="#"><i class="fa fa-chevron-left" aria-hidden="true"><span>Scroll left</span></i></a>',
    nextArrow:
      '<a href="#"><i class="fa fa-chevron-right" aria-hidden="true"><span>Scroll right</span></i></a>',
    slidesToShow: 3,
    slidesToScroll: 3,
    responsive: [
      {
  ...
  const loadData = (carouselId) => {
    const url = constants.shared.urls.bookCarousels.replace(
      "CAROUSEL_ID",
      carouselId
    );
  ...
  const authorExtractor = (textNode) =>
    $(textNode).text().split(",").slice(0, 2).join(",");
  ...
  const cleanHtml = (index, listItem) => {
    ...
    $image
      .attr("src", $image.attr("src").replace(/sc.gif/gi, "mc.gif"))
      .attr("title", "")
      .attr("style", "")
    ...
    if (isTitleSearch) {
      const author = authorExtractor(
        $listItem.find("div").eq(1).contents().filter(textNodeFilter)
      );
